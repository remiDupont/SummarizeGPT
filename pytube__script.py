...
def download_url(url):
    ...
    video.download(
        "./download",
        filename=yt.title.replace(" ", "_").replace(":", "").replace("&", "")[:20]
        + "."
...
def download_audio(url):
    ...
    # Sélection du flux audio de la meilleure qualité disponible
    audio_stream = yt.streams.filter(only_audio=True).first()

    # Afficher des détails sur la vidéo
    print("Titre :", yt.title)
    print("Longueur de la vidéo :", yt.length, "secondes")
    print("Bitrate de l'audio sélectionné :", audio_stream.abr)

    # Téléchargement de l'audio
    audio_stream.download(
        output_path="./download",
        filename=yt.title.replace(" ", "_").replace(":", "").replace("&", "")[:40]
        + "."
        + audio_stream.mime_type.split("/")[-1],
    )
